...
int example_yolov2_chain_search()
{
  ...
  if (op_idx > -1 && op_count > -1)
  {
    ...
    for (int i = op_idx; i < (op_idx + op_count) && i < static_cast<int>(params.size()); i++)
    {
...
int example_nn_model_chain_search(const std::string &model_name)
{
  ...
  const index_t total_raw_size = mace::CodlOpTaskChainHelper::CalcTotalMemoryUsage(op_chains);
